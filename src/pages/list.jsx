...
export default function Listbysearch(){
    ...
    const{pokemonstate:state,loading,type}=useSelector((state)=>state.pokereducer)
...
<Box  bgColor="whiteAlpha.900" backgroundPosition="center" background-size="cover" pt="50px">
            <Box  >
{type==="pokemon"?<Box  pt="20px" w="95%" m="auto" display="grid" gap="15px" justifyContent="center" gridTemplateColumns={breakpoints}>  <Link to={`/details/${state?.data?.id}`}> <Box className="green" cursor="pointer"   boxShadow='0 0 5px black' p='6' rounded='md' bg='white' bgColor={'white'} backgroundRepeat="no-repeat" background-size="cover">
       {loading?<Box textAlign="center"><Spinner thickness='4px'
         speed='0.65s'
         emptyColor='gray.200'
         color='blue.500'
         size='xl'/></Box>:<Box><Image  src={state?.data?.sprites?.other?.home?.front_default}/><Text textAlign="center" color={x} fontSize="20px" fontWeight="bold">{state?.data?.name}</Text></Box>}
